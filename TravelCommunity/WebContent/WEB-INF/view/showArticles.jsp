   ...
   <div>
      <center>
         <c:if test="${pageNav.prevPage}">
            <a href="<c:url value='/action/showArticles?todo=articleList&page=${pageNav.startPage - 1}' />">prev</a>
         </c:if>
         <c:forEach var="page" begin="${pageNav.startPage}" end="${pageNav.endPage}">
            <a href="<c:url value='/action/showArticles?todo=articleList&page=${page}' />">[${page}] </a>
         </c:forEach>
         <c:if test="${pageNav.nextPage}">
            <a href="<c:url value='/action/showArticles?todo=articleList&page=${pageNav.endPage + 1}' />">next</a>
         </c:if>
      </center>
